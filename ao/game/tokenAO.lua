...
--
if not Balances then Balances = { [ao.id] = tostring(bint(10000 * 1e3)) } end

if Name ~= 'AO Coin' then Name = 'AO Coin' end
...
Handlers.add('transfer', Handlers.utils.hasMatchingTag('Action', 'Transfer'), function(msg)
  ...
  if bint.__le(qty, balance) then
    ...
    if not msg.Cast then
      ...
      if (msg.BetOn ~= nil) then
        creditNoticeData['BetOn'] = msg.BetOn
      end
      if (msg.Stake ~= nil) then
        creditNoticeData['Stake'] = msg.Stake
      end

      ao.send(creditNoticeData)
    end
  else
    ao.send({
      Target = msg.From,
...
Handlers.add('mint', Handlers.utils.hasMatchingTag('Action', 'Mint'), function(msg)
  ...
  if msg.From == ao.id then
    ...
    ao.send({
      Target = msg.Recipient,
      Data = Colors.gray .. "Receive " .. Colors.blue .. msg.Quantity .. " $AO" .. Colors.reset
    })
  else
    ao.send({
      Target = msg.From,
